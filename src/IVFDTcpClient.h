...
signals:
    void SIG_ConnectToHost(QString addr, quint16 portn);
    void SIG_ClearClient();
    void SIG_HeartBeat();
    void SIG_SendMsgToServer(QByteArray cmd ,QByteArray val);
...
public slots:
    void SLOT_ReadMsg();
    void SLOT_NewConnect(QString addr, quint16 portn);
    void SLOT_DisplayError(QAbstractSocket::SocketError);
    void SLOT_ClearClient();
    void SLOT_HeartBeat();
    void SLOT_SendMsgToServer(QByteArray cmd, QByteArray val);
